...
class CameraManager: ObservableObject {
    ...
    private func setupPhotoOutput() {
        if session.canAddOutput(photoOutput) {
            ...
            photoOutput.maxPhotoDimensions = .init(width: 4032, height: 3024) // for ios 15.6 and the older versions
    ...
    func captureImage() {
        sessionQueue.async { [weak self] in
            ...
            if let videoConnection = photoOutput.connection(with: .video), videoConnection.isVideoRotationAngleSupported(CGFloat.pi) {
                videoConnection.videoRotationAngle = .pi
            }
